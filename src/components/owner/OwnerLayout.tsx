...
import {
  ...
  Sprout, Moon, Sun, Menu, X, Users, LineChart
} from "lucide-react";
...
const NAV = [
  ...
  { to: "/owner/reports", label: "Reports", icon: LineChart },
  { to: "/owner/settings", label: "Settings", icon: Settings },
] as const;

// Bottom tab bar on mobile only has room for a handful of items — the rest
// stay reachable via the sidebar and the mobile hamburger sheet (both list
// the full NAV above).
const MOBILE_TABS = NAV.filter((n) =>
  ["/owner/dashboard", "/owner/products", "/owner/staff", "/owner/sales-history", "/owner/reports"].includes(n.to)
);
    ...
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      >
        ...
        <div className="border-t border-border p-2">
          <HoverTip label={collapsed ? "Expand sidebar" : "Collapse sidebar"}>
            <button
